...
$voltar = "";
// Excluir
if (isset($_GET['id'])) { // Verifica se o botão excluir foi clicad
    
    $sqlVerificarAutor = "SELECT * FROM livroautor WHERE idAutor = " . $_GET['id'];
    $resultadoVerificarEmprestimo = mysqli_query($conexao, $sqlVerificarAutor);

    if (mysqli_num_rows($resultadoVerificarEmprestimo) > 0) {
        // O leitor possui empréstimos pendentes, não permitir a exclusão
        $mensagemAlert = "Não é possível excluir o Autor. Há livros cadastrados com ele.";
    } else {
        // Não existem empréstimos pendentes, prosseguir com a exclusão
        $sqlExcluirLeitor = "DELETE FROM autor WHERE id = " . $_GET['id'];
...
if (isset($_POST['pesquisar'])) { //se clicou no botao pesquisar
    ...
    $voltar = '<a href="listarAutores.php"><button name="voltar" stype="button" class="botaopesquisar">Voltar</button></a>';
}
...
<h1 class="titulo text">Listagem de Autores
    <a href="cadastrarAutores.php" class="botao">
